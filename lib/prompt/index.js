...
import { buildCharacterSheetPrompt, buildCloseUpPrompt, buildFeatureSheetPrompt } from './profileSheets.js'
...
export function buildPrompts(jobType, character, options = {}) {
  ...
  switch (jobType) {
    case JOB.CHARACTER_SHEET:
      ...
      // The wizard and the profile page word this differently; the profile
      // version wins when the character already exists.
      return [c.id ? buildCharacterSheetPrompt(c) : buildInfluencerSheetPrompt(c)]

    case JOB.CLOSE_UP:
      return [buildCloseUpPrompt(c)]

    case JOB.FEATURE_SHEET:
      return [buildFeatureSheetPrompt(c)]
