...
// ── APIMart candidates ───────────────────────────────────────────
// Same underlying models again, a third route to them. Listed LAST everywhere
// so automatic routing never reaches them: the request bodies are unverified
// (the account had no balance when the adapter was written), so APIMart runs
// only when explicitly chosen. Move these up the list once
// scripts/verify-apimart.mjs passes.
const AM_GPT_IMAGE = { provider: 'apimart', kind: 'image', models: { withRefs: 'gpt-image-2',  withoutRefs: 'gpt-image-2' } }
const AM_SEEDANCE  = { provider: 'apimart', kind: 'video', models: { withRefs: 'seedance-2.0', withoutRefs: 'seedance-2.0' } }
...
export const ROUTES = {
  ...
  // Identity-card headshot and the macro detail grid. Both are reference
  // assets every later photo depends on, so they take the strongest model.
  [JOB.CLOSE_UP]: [
    { ...GPT_IMAGE_2,      params: { quality: 'high' } },
    { ...HF_GPT_IMAGE },
    { ...AM_GPT_IMAGE },
  ...
  //
  // generateAudio matches Seedance's own default of true. It was set to false
  // here originally, which produced silent clips that looked like the provider
  // lacked audio support — it does not.
  [JOB.VIDEO]: [
    { provider: 'kie', kind: 'video', models: { withRefs: 'bytedance/seedance-2',      withoutRefs: 'bytedance/seedance-2' },      params: { resolution: '720p', generateAudio: true } },
    { provider: 'kie', kind: 'video', models: { withRefs: 'bytedance/seedance-2-fast', withoutRefs: 'bytedance/seedance-2-fast' }, params: { resolution: '720p', generateAudio: true } },
    { ...HF_SEEDANCE, params: { resolution: '1080p' } },
    { ...AM_SEEDANCE, params: { resolution: '720p' } },
  ],
}
