...
function unwrap(body) {
  if (body?.code !== undefined && body.code !== 200) {
    const message = body.msg || `API code ${body.code}`
    // kie.ai reports rejected parameters as a 500, which looks transient and
    // isn't. Retrying is pointless and failing over is worse: it silently runs
    // the job on a different model, which is how a close-up came back as a
    // completely different person instead of an error.
    throw new ProviderError(NAME, message, {
      status: body.code,
      retryable: body.code >= 500 && !isParameterRejection(message),
    })
  }
...
export async function uploadMedia(dataUrl, { type = 'image' } = {}) {
  // An http(s) reference is COPIED into kie's own store, not passed through.
  //
  // This is the fix for a character sheet coming back as a completely different
  // person. Handing the model a foreign URL is unreliable: generation URLs
  // expire after 24 hours, and when the model cannot fetch the face it does not
  // error — it quietly generates from the text prompt alone and returns a
  // stranger. Copying the bytes makes the reference actually present.
  //
  // Done server-side because the browser is blocked by CORS on those hosts.
  if (/^https?:\/\//.test(dataUrl || '')) {
    const body = await httpJson(`${UPLOAD_BASE}/file-url-upload`, {
      method: 'POST',
      headers: headers(),
      body: JSON.stringify({ fileUrl: dataUrl, uploadPath: `influencer-studio/${type}` }),
    }, { provider: NAME, timeoutMs: 60000 })

    const d = unwrap(body)
    const copied = d?.downloadUrl || d?.fileUrl || d?.url || d?.data?.downloadUrl
    if (!copied) throw new ProviderError(NAME, `URL upload returned no URL: ${JSON.stringify(d).slice(0, 200)}`)
    return copied
  }
...
export async function createImageJob({ model, prompt, refUrls = [], aspectRatio = '9:16', resolution = null, quality = null, auth: _auth = null }) {
  const input = { prompt, aspect_ratio: normalizeAspect(model, aspectRatio) }
...
export async function createVideoJob({
  ...
  aspectRatio = '9:16', resolution = '720p', duration = 5, generateAudio = false, auth: _auth = null,
}) {
  const input = {
    prompt,
...
// Second arg carries the user's provider auth for providers that need it.
// kie.ai uses our server key, so it is accepted and ignored.
